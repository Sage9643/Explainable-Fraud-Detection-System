...
export default function CSVUploader({ onFileSelected, disabled = false, accept = ".csv" }) {
  ...
  return (
    ...
    >
      <div className="flex h-11 w-11 items-center justify-center rounded-full bg-brand-50 text-brand-600 dark:bg-brand-700/20 dark:text-brand-400">
      ...
      <div>
        ...
        <p className="text-xs text-ink-500 dark:text-ink-400">or click browse to choose a file</p>
      </div>
      <button
        type="button"
        disabled={disabled}
        onClick={() => inputRef.current?.click()}
        className="rounded-md bg-brand-500 px-4 py-2 text-xs font-medium text-white transition-colors hover:bg-brand-600 disabled:opacity-50"
      >
        Browse Files
      </button>
      <input
